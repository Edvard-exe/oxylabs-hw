...
class InformationRetrieval: 
    ...
    def final_bot(self, results_with_urls: List[Dict[str, Any]]) -> tuple[str, List[str]]:
        ...
        - Format code blocks with proper language syntax highlighting: ```language
        - Use bullet points for lists
        - Use headers (##, ###) to organize sections
        - Format inline code with backticks: `code`
        - Use **bold** for emphasis on important points
        - Ensure code examples are properly indented and readable
        ...
        unique_urls = list(set([result['url'] for result in results_with_urls if result['url']]))
        
        return response.content, unique_urls

    def orchestrate_retrieval(self) -> tuple[str, List[str]]:
